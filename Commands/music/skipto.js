...
module.exports = {
        data: new SlashCommandBuilder()
            ...
            .addIntegerOption(option => option.setName('position').setRequired(true).setDescription('The position'))
        ...
        async execute(interaction, client) {
            ...
            try {
                let position = parseInt(interaction.options.getString("position"))
       ...
       .setAuthor({name: style.errorText, iconURL: style.errorImg})
         .setDescription(`${style.error} I am missing the **\`CONNECT\`** permission`)]})
                    ...
                    if(!voiceChannel) return interaction.reply({embeds: [new EmbedBuilder()
                        ...
                        let song = await queue.jump(position)

                   
                    interaction.reply({embeds: [new EmbedBuilder()
                        .setColor(style.color)
                        .setThumbnail(song.thumnail)
                        .setDescription(`${style.skip} Skipped to position **\`${position}\`**, now playing **\`${song.name}%\`**`)]})
                        ...
                        interaction.reply({embeds: [new EmbedBuilder()
                            .setColor(style.errorColor)
                            .setTitle("An error occured")
                            .setDescription(`${style.error} I could probaly not be in a voice channel, or the specified position is non-existent
